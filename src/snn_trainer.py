...
class SequenceNeuralNetworkTrainer:
    ...
    def val_fn(self, val_loader, model, criterion, device):
        ...
        with torch.no_grad():
            for features, labels in progress_bar:
                ...
                targets += labels.view(-1).detach().cpu().numpy().tolist()
    ...
    def train_and_validate(self, df_train, df_test):
        ...
        print(f'\n{"-" * 30}\nRunning Sequence Neural Network Model for Training\n{"-" * 30}\n')
        scores = {}
        test_dataset = SequenceDataset(df_test.loc[:, self.features].values, sequence_length=self.model_parameters['sequence_length'])
        ...
        for fold in range(1, 5):
            ...
            train_dataset = SequenceDataset(df_train.loc[trn_idx, self.features].values, df_train.loc[trn_idx, self.target].values, sequence_length=self.model_parameters['sequence_length'])
            ...
            if len(val_idx) > 0:
                val_dataset = SequenceDataset(df_train.loc[val_idx, self.features].values, df_train.loc[val_idx, self.target].values, sequence_length=self.model_parameters['sequence_length'])
        ...
        print(f'{"-" * 30}\nSequence Neural Network Mean Validation Score: {np.mean(list(scores.values())):6f} (±{np.std(list(scores.values())):2f})\n{"-" * 30}\n')
